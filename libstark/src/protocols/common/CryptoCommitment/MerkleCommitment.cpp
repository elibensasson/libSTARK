...
void hash(void const* const src, void* const dst) {
	...
	// Code for BLAKE2s
	//
	blake2s((uint8_t*)dst, src, Blake2::key, BLAKE2S_OUTBYTES, 0,
		BLAKE2S_KEYBYTES);
}
...
std::string hashDigest_t::toString() const {
	...
	for (unsigned short i = 0; i < sizeof(hashDigest_t) / sizeof(short);
	     i++) {
		stream << std::setfill('0') << std::setw(sizeof(short) * 2)
		       << shortsBuff[i] << " ";
	}
...
unsigned short getDualBlockSize() { return 2 * getBlockSize(); }
...
//
hashDigest_t constructMerkleTree(void const* const src, const short src_logLen,
				 void* const dst) {
	using Infrastructure::POW2;

	short curr_dst_logLen = src_logLen - logBytesPerHash - 1;
	hashDigest_t* curr_src = (hashDigest_t*)src;
	_COMMON_ASSERT(src_logLen > logBytesPerHash,
		       "It is assumed the source length contains a power of 2 "
		       "blocks of 256 bits, src_logLen = " +
			   std::to_string(src_logLen));

	while (curr_dst_logLen >= 0) {
		hashDigest_t* curr_dst =
		    ((hashDigest_t*)dst) + (1UL << curr_dst_logLen);
...
hashDigest_t getMerkleCommitmentInplace(void* dataInp, const short src_logLen) {
	...
	for (unsigned short currStepLog = 1; currStepLog <= logLen;
	     currStepLog++) {
...
//
// Constructs a Merkle sub-tree for a sigment in the src buffer (srcLen expected
// in bytes) The sub - tree is written to dst It is expected src_logLen is in
// bytes. It is expected the size of dst in bytes is at least srcLen.
//
void constructMerkleSubTree(void const* const src, const short src_logLen,
			    const size_t sigment_logLen,
			    const size_t sigment_index, void* const dst) {
	using Infrastructure::POW2;

	short curr_dst_row_logLen = src_logLen - logBytesPerHash - 1;
	short curr_sigment_logLen = sigment_logLen - logBytesPerHash - 1;
	hashDigest_t* curr_src_shifted =
	    (hashDigest_t*)src +
	    POW2(sigment_logLen - logBytesPerHash) * sigment_index;

	_COMMON_ASSERT(sigment_logLen > logBytesPerHash,
		       "It is assumed the source length contains a power of 2 "
		       "blocks of 256 bits");

	while (curr_sigment_logLen >= 0) {
		hashDigest_t* curr_dst_row =
		    ((hashDigest_t*)dst) + POW2(curr_dst_row_logLen);
		hashDigest_t* curr_dst_shifted =
		    curr_dst_row + POW2(curr_sigment_logLen) * sigment_index;
		const size_t curr_sigment_len = POW2(curr_sigment_logLen);
#pragma omp parallel for
		for (unsigned long long i = 0; i < curr_sigment_len; i++) {
			hash(curr_src_shifted + (i << 1UL),
			     curr_dst_shifted + i);
		}
...
path_t getPathToBlock(void const* const tree, const short src_logLen,
...
vector<path_t> getPathToBlocksInPlace(void* dataInp, const short src_logLen,
				      const vector<size_t>& blockIndices) {
	using Infrastructure::POW2;
	_COMMON_ASSERT(src_logLen > logBytesPerHash,
		       "It is assumed the source length contains a power of 2 "
		       "blocks of 256 bits, src_logLen = " +
...
#pragma omp parallel for
		...
		// collect path data
		{
			...
			for (unsigned int q = 0; q < blockIndices.size(); q++) {
				const size_t currLayerBlock =
				    blockIndices[q] >> (currStepLog - 1);
...
bool verifyPathToBlock(void const* const blockData, const hashDigest_t& root,
		       const path_t& path, const size_t blockIndex) {
...
SparceMerkleLayer SparceMerkleLayer::calculateNextLayer(
    const SparceMerkleLayer& received) const {
...
std::vector<hashDigest_t> SparceMerkleTree::toVector() const {
	const auto serializationMapping =
	    getSerializationMapping(layers_[0].getIndices());
...
void SparceMerkleTree::DeSerialize(
    const std::set<size_t>& queriedIndices,
    const std::vector<hashDigest_t>& serializedSubtree) {
	const auto serializationMapping =
	    getSerializationMapping(queriedIndices);
	_COMMON_ASSERT(serializationMapping.size() == serializedSubtree.size(),
		       "The serialization length is not as expected : " +
			   std::to_string(serializationMapping.size()) +
			   " != " + std::to_string(serializedSubtree.size()));
	for (unsigned int i = 0; i < serializationMapping.size(); i++) {
		layers_[serializationMapping[i].first].addEntry(
		    serializationMapping[i].second, serializedSubtree[i]);
	}
}

std::vector<std::pair<short, size_t> >
SparceMerkleTree::getSerializationMapping(
    const std::set<size_t>& queriedIndices) const {
	...
	for (unsigned short layerIdx = 0; layerIdx < layers_.size();
	     layerIdx++) {
		std::map<size_t, bool> next_knownIndices;
		for (const auto& known : knownIndices) {
			if (known.second == true) {
				// If current data is explicit in subtree, it
				// must be fetched from serialization
				res.push_back(std::pair<short, size_t>(
				    layerIdx, known.first));
			}
		...
		// Any element in next known indices that is known must have its
		// mate known as well, thus if it is not known implicitly it
		// must be passed explicitly in the serialization
...
void SparceMerkleTree::addPath(const std::array<hashDigest_t, 2>& data,
			       const path_t& path, const size_t pairIdx) {
